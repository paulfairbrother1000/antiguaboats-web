...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("charter_types")
    .select("slug,title,base_price_cents,currency,active")
    .eq("active", true);

  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  const by_slug: Record<string, { title: string; base_price_cents: number; currency: string }> = {};
  ...
  return NextResponse.json({ by_slug });
}
