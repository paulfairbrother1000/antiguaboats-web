...
type ApiDay = { date: string; slots: { slotId: string; label: string; start: string; end: string; available: boolean }[] };
...
export default function AvailabilityPage() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch(`/api/availability?charter=${encodeURIComponent(charter)}&from=${from}&to=${to}`, { cache: "no-store" });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      ...
      <p className="mt-2 text-slate-600">
        Select a date and slot. Availability accounts for both Antigua Boats bookings and Pace Shuttles busy time for Silver Lady.
      </p>

      <div className="mt-6 flex flex-col sm:flex-row gap-3 items-end">
        <div>
          <label className="block text-sm text-slate-600">From</label>
          <input className="mt-1 border rounded-xl px-3 py-2" type="date" value={from} onChange={e => setFrom(e.target.value)} />
        </div>
        <div>
          <label className="block text-sm text-slate-600">To</label>
          <input className="mt-1 border rounded-xl px-3 py-2" type="date" value={to} onChange={e => setTo(e.target.value)} />
        </div>
        <button
          onClick={load}
      ...
      <div className="mt-8 space-y-4">
        {days.map(d => (
          <div key={d.date} className="rounded-2xl border bg-white p-5">
            ...
            {d.slots.length === 0 ? (
              <div className="mt-2 text-sm text-slate-600">This charter is on request. Please use Contact to arrange.</div>
            ) : (
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {d.slots.map(s => (
                  ...
                  >
                    ...
                    <div className={`mt-2 text-sm font-medium ${s.available ? "text-emerald-700" : "text-slate-500"}`}>
