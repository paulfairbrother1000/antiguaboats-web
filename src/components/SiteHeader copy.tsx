  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur border-b">
      <div className="mx-auto max-w-6xl px-4 py-2 flex items-center justify-between gap-3">
        {/* Desktop nav centered now logo removed */}
        <nav className="hidden md:flex items-center gap-1 mx-auto">
          ...
          <Link
            href="/booking"
            className="ml-2 inline-flex items-center rounded-xl bg-sky-600 px-5 py-2 text-sm font-semibold text-white hover:bg-sky-700"
          >
            Book Now
          </Link>
        </nav>
        ...
        {/* Mobile hamburger */}
        <button
          className="md:hidden inline-flex items-center justify-center rounded-xl border px-3 py-2 text-slate-700 hover:bg-slate-50 ml-auto"
          onClick={() => setOpen((v) => !v)}
          aria-label="Menu"
        >
